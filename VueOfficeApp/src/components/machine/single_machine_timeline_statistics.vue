...
<template>
	<div id="app">
		<div class="content">
			...
			<div id="slider"></div>
				<div class="look" style='margin-top: 1rem;text-align: center;'> 
				查看 <el-time-picker v-model="value2" class='lookTime start' :picker-options="{ selectableRange: '00:00:00 - 23:59:59'}" placeholder="开始时间">
  					</el-time-picker> -
  					<el-time-picker v-model="value3" class='lookTime end' :picker-options="{ selectableRange: '00:00:00 - 23:59:59'}" placeholder="结束时间">
  					</el-time-picker>
				<!-- <input  readonly="" name="input_date" class='lookTime start' placeholder="选择开始时间" >-<input name="input_date" class='lookTime end' placeholder="选择结束时间"> 的数据  -->
				<p class="pBtn"><a href="javascript:;" class="button button-round" id='lookBtn'>查看</a></p>
			</div>
			<div class="look2" style='margin-top: 1rem;text-align: center;display:none;;'> 
				查看  <el-time-picker v-model="value4" class='lookTime start2' :picker-options="{ selectableRange: '00:00:00 - 23:59:59'}" placeholder="开始时间">
  					</el-time-picker> -
  					<el-time-picker v-model="value5" class='lookTime end2' :picker-options="{ selectableRange: '00:00:00 - 23:59:59'}" placeholder="结束时间">
  					</el-time-picker>
				<!-- <input  readonly="" name="input_date" class='lookTime start2' placeholder="选择开始时间" >-<input name="input_date" class='lookTime end2' placeholder="选择结束时间"> 的数据  -->
	     ...
	     export default {
	            ...
	            mounted:function(){
						...
						function selectTime(timelineDate,dayNumber) {
							$("#lookBtn2").click(function(){
									...
									var newTime=nowToday;			
							       	...
							       	var lookStart =vueThis.value4.getHours();
							       	var lookStart2=vueThis.value4.getMinutes() ;
							       	var lookStart3=vueThis.value4.getSeconds() ;
							       	var lookEnd =vueThis.value5.getHours();
							       	var lookEnd2=vueThis.value5.getMinutes() ;
							       	var lookEnd3=vueThis.value5.getSeconds() ;
							       	// 填写的开始时间（秒）
							       	var lookStartTime=timelineDate+lookStart*3600+lookStart2*60+lookStart3-28800; 
							       	// 填写的结束时间（秒）
							        var lookEndTime=timelineDate+lookEnd*3600+lookEnd2*60+lookEnd3-28800;
							       ...
							       if(lookStart<timeHour){
							       	...
							       	}else if(lookStart>=timeHour&lookEnd>=timeHour&&lookStartTime<lookEndTime) {
								       		var startTime=lookStart*3600+lookStart2*60+lookStart3+timelineDate-28800;
									        var endTime=lookEnd*3600+lookEnd2*60+lookEnd3+timelineDate-28800;
			       ...
			       $("#lookBtn").click(function(){
			        var newDate=new Date(),newHour=newDate.getHours()*3600,newMinutes=newDate.getMinutes()*60,newSeconds=newDate.getSeconds() ,newTime=newHour*3600+newMinutes*60+newSeconds+getTodayStartUnixTimestamp(); 
			       	var timeHour=(timelineDate-getTodayStartUnixTimestamp())/3600;
			       	var lookStart =vueThis.value2.getHours();
			       	var lookStart2=vueThis.value2.getMinutes();
			       	var lookStart3=vueThis.value2.getSeconds();
			       	var lookEnd =vueThis.value3.getHours();
			       	var lookEnd2=vueThis.value3.getMinutes();
			       	var lookEnd3=vueThis.value3.getSeconds();
			       	// 填写的开始时间（秒）
			       	var lookStartTime=getTodayStartUnixTimestamp()+lookStart*3600+lookStart2*60+lookStart3; 
			       	// 填写的结束时间（秒）
			        var lookEndTime=getTodayStartUnixTimestamp()+lookEnd*3600+lookEnd2*60+lookEnd3;
			        // 判断开始和结束时间是否为空
			         if(lookStart<timeHour){
			       		lookStartTime+=86400
			       		lookStart+=24
			       	...
			       	if(lookEndTime>newTime||lookStartTime>newTime){
			       		vueThis.$message('时间还没到，请选择正确时间')
				       		...
				       		var startTime=lookStart*3600+lookStart2*60+lookStart3+getTodayStartUnixTimestamp();
					        var endTime=lookEnd*3600+lookEnd2*60+lookEnd3+getTodayStartUnixTimestamp();
					        var timeLength= parseInt(new Date().getTime() / 1000, 10)-timelineDate;
					        var start=((startTime-timelineDate)/ timeLength)*100;
					        var  end=((endTime-timelineDate)/timeLength)*100;
							slider.noUiSlider.set([start, end])	
