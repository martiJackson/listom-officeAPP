    ...
    <div id='app'>
        <div class="content">
	        <div class="buttons-tab fixed-tab buttons-fixed">
	            <a href="#tab1" class="tab-link button active">加班记录</a>
	            <a href="#tab2" class="tab-link button">加班申请</a>
	            <a id="tab-check-leaves" href="#tab3" class="tab-link button">加班审核</a>
	        </div>
	        <div class="tabs">
	        	 <!-- 加班记录列表的模板 -->
	                ...
	                <div class="content-block tab-page-block" id="tab-page-list">
	                	<div class="list-block media-list" >
	                		...
	                		<ul v-for="lis,index of list">
	                			<li class="item-content item-link list-item" v-for="lis,index of list" @click="listItem(index)" >
							        ...
							        <div class="item-inner">
							        	<div class="item-title" id="title">标题：{{lis.title}}</div>
								        <div class="item-title" id="type" >类型：{{lis.type}}</div>
	                                    ...
	                                    <div class="item-inner">
	                                        ...
	                                        <div class="item-input">
	                                            <!-- <input type="text" placeholder="选择加班类型" id="select-type"> -->
	                                             <el-autocomplete class="inline-input" v-model="state" :fetch-suggestions="querySearch" placeholder="选择加班类型" ></el-autocomplete>
	                                        </div>
	                                    </div>
	                                </div>
	                            </li>
	                            <li>
	                                <div class="item-content">
	                                    <div class="item-inner">
	                                        ...
	                                        <div class="item-input">
	                                            <textarea id="leave-reason" placeholder="填写加班原因"></textarea>
	                                        </div>
	                                    </div>
	                                </div>
	                            </li>
	                                ...
	                                <div class="item-content">
	                                    <div class="item-inner">
	                                        ...
	                                        <div class="item-input">
	                                            ...
	                                            <template>
												    <el-date-picker v-model="value1" id='val' type="date" placeholder="选择开始日期" :picker-options="pickerOptions0">
												    </el-date-picker>
												</template>
	                                        </div>
	                                    </div>
	                                    ...
	                                    <div class="item-inner">
	                                        ...
	                                        <div class="item-input">
	                                            <!-- <input type="text" placeholder="请选择结束时间" id="end-time-picker" class="date-picker"> -->
	                                             <template>
												    <el-date-picker v-model="value2"  id='val' type="date" placeholder="选择开始日期" :picker-options="pickerOptions0">
												    </el-date-picker>
												</template>
	                                        </div>
	                                    </div>
	                ...
	                <div class="content-block tab-page-block" id="tab-page-check-content" >
		                <div v-for='lis2,index of list2'>
		                	<div class="card" :id="'card-id-'+lis2.id">
						        <div class="card-header orange-color">{{lis2.user_name}}的加班申请</div>
...
#tab-page-check-content .hint{
	margin-top:2.4rem;
}
#card-id-89{
	margin: 2rem .5rem .5rem .5rem;
}
div.card-header.orange-color{
	text-align: center;
}
</style>
<script type="text/javascript">
  export default {
	...
	$('#btn-submit').click(function() {
	    ...
	    var startTime = vueThis.alertstate().start;
	    var endTime = vueThis.alertstate().end;
	    var hours = $('#hours').val();
	    ...
	    if (type=='') {
	    	// alert('请选择请假类型')
	    	vueThis.$message('请选择加班类型')
	        // $.toast('请选择请假类型');
	        return;
	    }
	    if (isEmpty(reason)) {
	    	// alert('请填写请假原因')
	    	vueThis.$message('请填写加班原因')
	        // $.toast('请填写请假原因');
	        return;
	    }
	    if (isEmpty(startTime) || isEmpty(endTime)) {
	    	// alert('请选择请假时间')
	    	vueThis.$message('请选择加班时间')
	        // $.toast('请选择请假时间');
	        return;
	    }
	    ...
	    if (isEmpty(hours)) {
	    	// alert('请填写请假小时')
	    	vueThis.$message('请填写加班小时')
	        // $.toast('请填写请假小时');
	        return;
	    }
	    if (!isNumber(hours) || hours <= 0) {
	    	// alert('请假小时数填写有误')
	    	vueThis.$message('加班小时数填写有加班误')
	        // $.toast('请假小时数填写有误');
	        return;
	    }
