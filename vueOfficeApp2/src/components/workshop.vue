    ...
    <div id="app">
        <div class="content" id='content'>
            ...
            <div class="list-block">
                ...
                <div class="row"  v-for='lis,index of list' :id='"machine-list-"+lis.id' hidden="hidden" ></div>
            </div>
            ...
            <div class="list-block">
                ...
                <div class="row" v-for='lis2,index of list2' :id="'alert-machine-list-'+lis2.id" hidden="hidden"></div>         
            </div>
        </div>
        <!-- 车间分组的popup -->
       <!--  <el-dialog title="提示" v-model="dialogVisible" class="popup-group" id="dialog"> -->
       <div class="popup-group popup-group-workshop " id="dialog"   style='display:none;position:fixed;left:0;height:100%;width:100%;background:#fff;z-index:100000'>
         ...
         export default {
            ...
            function showGroupMachines(groupId, list) {
                ...
                if(isClickedWorkshopList) {   // 车间列表
                    ...
                    for(var i=0;i<list.length;i++){
                         html+='<div class="col-50 longTouch">'+
			...
			window.onload=function(){
				if(typeof(window.js_interface) == "undefined"){
					...
					phone=window.js_interface.getPhoneNumber();
					window.localStorage.setItem('phone',phone)
				}
			};
			// 获取客户账号，识别不同客户
            window.localStorage.setItem(phone+'-'+'getID','5-20');            
            var lessenArr=[],addArr2=[],getArr=[],onceIn=true,getID=[];
            // 获取修改的ID
            function getId(){
                if(typeof(window.js_interface) == "undefined"){
                    if(window.localStorage.getItem(phone+'-'+'getID')!=null){	                   
	                    getArr=window.localStorage.getItem(phone+'-'+'getID').split(',');    	                    
	                   }else{

	                   }                                                               
                }else{
                     try {
                            getArr=window.js_interface.getMachineId().split(',');
                            window.localStorage.setItem(phone+'-'+'getArr',getArr.join(''));
                             // alert(window.js_interface.getMachineId())
                        }catch (e) {
                            console.log(e);
                        }       
                }
                if(getArr!=null){
                    getArr.forEach(function(e){
                            $('#'+e).parent('.longTouch').append("<p class='cardCover'></p>");
                            $('#'+e).parent('.longTouch').addClass("toggle");
                     });
                }
            };
            // 传递取消关注的ID
            function lessen(ID){
                if(typeof(window.js_interface) == "undefined"){
                    ...
                    getArr=window.js_interface.getMachineId();
                    window.localStorage.setItem(phone+'-'+'getArr',getArr)
                }
            };
            // 传递关注的ID
            function add(ID){
